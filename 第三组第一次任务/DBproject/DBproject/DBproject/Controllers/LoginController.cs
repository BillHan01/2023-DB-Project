...
namespace DBproject.Controllers
{
    ...
    public class LoginController : ControllerBase
    {
        ...
        public IActionResult PostLogin([FromBody] LoginModel model)
        {
            ...
            {
                message = DBproject.DataBase.oracleCon.IDLogin(model.user_ID_phonenumber, model.user_password);
            }
            else if(model.login_type == "1")//电话登录
            {
                message = DBproject.DataBase.oracleCon.PhonenumberLogin(model.user_ID_phonenumber, model.user_password);
            }
