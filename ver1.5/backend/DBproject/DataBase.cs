    ...
    public class DataBase
    {
        ...
        public List<CommodityListModel> sqlSearchCommodityByName(searchCommodityModel model)
        {
            ...
            var searchSql =
                ...
                $" WHERE COM_NAME LIKE '%{model.search_str}%'  AND COM_STATUS !=-1";
            ...
            foreach (var ind in model.indent_arr)
            {
                ...
                foreach (var com_ind in ind.com_arr)
                {
                    var sql = $"Insert into INDENT (IND_ID,IND_QUANTITY,IND_MONEY,IND_CREATIONTIME,IND_VERIFICATIONCODE,IND_NOTES,IND_STATE,IND_RATING,CUS_ID,COM_ID,IND_RTIME,IND_RNOTES,IND_RMONEY) " +
                        $"values (IND_ID_SEQ.NEXTVAL,{com_ind.ind_quantity},{com_ind.ind_money},TO_DATE('{DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")}', 'YYYY-MM-DD HH24:MI:SS'),'{newGuid.ToString()}','{ind.ind_notes}',0,0,{model.cus_id},{com_ind.com_id},TO_DATE('0001-01-01 00:00:00', 'YYYY-MM-DD HH24:MI:SS'),0,-1)";
                    try
                    {
        ...
        public SendCommentModel sqlGetSingleCmt(CommentModel model)
        {
            ...
            using (OracleCommand command = con.CreateCommand())
            {
             ...
             $"RETURNING CMT_ID INTO :LastInsertedId";
                ...
                command.Parameters.Add("cmtContent", OracleDbType.Varchar2, model.cmt_content, ParameterDirection.Input);
