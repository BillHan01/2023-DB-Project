<template>
    ...
    <el-table
     ...
     :data="goodsList"
      ...
      >
      <el-table-column label="用户图片" width="200">
      ...
      <el-table-column prop="USER_ID" sortable="custom" label="用户ID">
        <template #default="scope">
        <el-tag @click="router.push({path:'/UserInfoPage',query:{id:goodsList[scope.$index].USER_ID}})">{{ goodsList[scope.$index].USER_ID }}</el-tag>
        </template>
      </el-table-column>
    <el-table-column prop="USER_REGTIME" label="用户注册日期" >
    </el-table-column>
      <el-table-column prop="USER_STATE" width="200" label="用户状态">
      </el-table-column>

      <el-table-column label="操作" width="200" v-if= "parseInt(type)">
      <template #default="scope">
        <el-button
          type="primary"
          icon="search"
