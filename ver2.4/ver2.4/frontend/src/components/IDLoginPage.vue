  ...
  const login_id = async ()=> {
          ...
          .then(response => {
            ...
            if(response.data.message==='该用户已经被封禁') {
                ...
                if (response.data.user_type=='1'){
                  ...
                  /*获取用户位置*/
                  axios.get(address + 'getinformation/user', { params: {user_ID: response.data.user_ID } })
                  .then((res) => {
      ...
      const fetchLocationData = () => {
        return new Promise(async (resolve, reject) => {
          try {
            const response = await axios.get(address + 'search/sendLocation');
    ...
    const sendSortedDataToBackend = async (sortedStoIds, userId) => {
      try {
        const response = await axios.post(address + 'search/getLocationSort', {
          sto_id: sortedStoIds,
          user_id: userId
        });
