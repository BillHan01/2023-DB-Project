<template>
    <el-container>
      ...
      <el-main>
        <el-form :model="userInfo" style="max-width: 460px;position: relative;margin: auto;">
          ...
          <el-form-item label="用户类型">
            ...
            <!-- ...additional form items for customers... -->
            ...
            <!-- additional form items for store -->
            ...
            <el-button type="primary" @click="modifyUserInfo">修改信息</el-button>
            <el-button type="success" @click="rechargeNow">充值</el-button>
            <el-button type="success" @click="withdrawNow">提现</el-button>
  ...
  const confirmRecharge = () => {
    ...
    axios.post(address+'balance/setBalance', {
      cus_id: user_ID.value,
  ...
  onMounted(async () => {
    ...
    try {
      const response = await axios.get(address+'getinformation/user', { params: {user_ID:user_ID.value } });
      ...
      if (userInfo.value.user_type === '0') {
        const customerResponse = await axios.get(address+'getinformation/customer', { params: { cus_ID: user_ID.value } });
        ...
        const storeResponse = await axios.get(address+`getinformation/store`, { params: { sto_ID: user_ID.value } });
        image.value='http://localhost:5000\\'+storeResponse.data.sto_licenseImg;
        if (storeResponse.status === 200) {
            Object.assign(userInfo.value, storeResponse.data);
            const storePicResponse = await axios.get(address+`getinformation/storeimg`, { params: { sto_ID: user_ID.value } });
